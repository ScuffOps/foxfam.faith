-- Privacy-safe public game progression for authenticated profile and Quarters visits.
-- Depends on: 20260718200316_starfishing_phase_2_progression.sql
-- Depends on: 20260722320000_add_game_achievement_collectibles.sql

begin;

do $$
begin
  if pg_catalog.to_regclass('public.user_fishpedia') is null
    or pg_catalog.to_regclass('public.user_relic_charms') is null
    or pg_catalog.to_regclass('public.user_trophies') is null
    or pg_catalog.to_regclass('public.achievement_catalog') is null then
    raise exception using
      errcode = '55000',
      message = 'Game progression migrations must be applied before the public projection';
  end if;
end
$$;
...
begin
    ...
    pg_catalog.jsonb_build_object(
      ...
      'source', pg_catalog.jsonb_build_object(
        'type', case
          ...
          when charm.data ->> 'source' = 'relic_roll' then 'relic-roll'
          else 'legacy'
        end,
        'key', case
  ...
  select
    pg_catalog.max(achievement.reward #>> '{effects,profile_frame}'),
    pg_catalog.max(achievement.reward #>> '{effects,profile_particle}')
  into profile_frame, profile_particle
  from public.user_relic_charms as charm
  ...
  if profile_frame !~ '^[a-z0-9-]{1,80}$' then
    profile_frame := null;
  end if;
  if profile_particle !~ '^[a-z0-9-]{1,80}$' then
